...
class WindowApproach(object):
    ...
    def __init__(self, w_size, step_size, replicon_dict, max_proc, norm_method,
            ...
            padj_threshold, mad_multiplier, fc_cutoff, pairwise_replicates,
            output_folder):
    ...
    def generate_window_counts(self):
        ...
        t_start = time()
        with futures.ProcessPoolExecutor(max_workers=self._max_proc
                ) as executor:
            future_to_lib_name = {executor.submit(lib.count_reads_for_windows):
    ...
    def perform_g_test_with_repl_for_windows(self):
        ...
        padj_values = p_values.loc[:, ["pooled_G_p_value", "total_G_p_value"]
                ].apply(self._correct_p_values, axis = 0)
        ...
        significance = pd.concat([p_values, padj_values],
                axis = 1).apply(self._check_significance_with_repl, axis = 1)
    ...
    def combine_peaks_and_recalculate_values(self):
        for replicon in self._replicon_dict:
            ...
            peak_df_forward = self._combine_windows(
                    self._window_df[(self._window_df["replicon"] == replicon
                    ) & (self._window_df["strand"] == "+")])
            if not peak_df_forward.empty:
                peak_df_forward["peak_strand"] = '+'
            # reverse strand
            peak_df_reverse = self._combine_windows(
                    self._window_df[(self._window_df["replicon"] == replicon
                    ) & (self._window_df["strand"] == "-")])
        ...
        for replicon in sorted(self._replicon_dict):
            for lib_name, lib in self._lib_dict.items():
                self._replicon_dict[replicon]["peak_df"][lib_name
                        ] = lib.replicon_dict[replicon]["peak_counts"]
            # add pseudocounts
            self._replicon_dict[replicon]["peak_df"][
                    self._lib_names_list] += 1.0
            # normalize counts
            self._replicon_dict[replicon]["peak_df"][self._lib_names_list
                    ] = self._replicon_dict[replicon]["peak_df"][
                    self._lib_names_list].div(self._size_factors,
                    axis='columns')
            if len(self._exp_lib_list) > 1:
                self._replicon_dict[replicon]["peak_df"
                        ] = self._perform_g_test_with_repl_for_peaks(
                        self._replicon_dict[replicon]["peak_df"])
            else:
                self._replicon_dict[replicon]["peak_df"
                        ] = self._perform_g_test_without_repl_for_peaks(
                        self._replicon_dict[replicon]["peak_df"])
            # calculate base means for all peaks
            self._replicon_dict[replicon]["peak_df"]["base_means"
                    ] = self._replicon_dict[replicon]["peak_df"].loc[:, 
                    self._lib_names_list].mean(axis = 1)
            # calculate fcs for all peaks
            self._replicon_dict[replicon]["peak_df"]["fold_change"] = (
                    self._replicon_dict[replicon]["peak_df"].loc[:, 
                    self._exp_lib_list].sum(axis = 1) /
                    self._replicon_dict[replicon]["peak_df"].loc[:,
                    self._ctr_lib_list].sum(axis = 1))
            
    def _generate_peak_counts(self):
        # execute read counting in parallel
        with futures.ProcessPoolExecutor(max_workers=self._max_proc
                ) as executor:
            future_to_lib_name = {executor.submit(lib.count_reads_for_peaks):
                    lib.lib_name for lib in self._lib_dict.values()}
        for future in futures.as_completed(future_to_lib_name):
            lib_name = future_to_lib_name[future]
            try:
    ...
    def write_output(self):
        ...
        for replicon in self._replicon_dict:
            ...
            self._replicon_dict[replicon]["peak_df"].reset_index(drop=True,
                    inplace=True)
    ...
    def generate_normalized_wiggle_files(self):
        ...
        with futures.ProcessPoolExecutor(max_workers=self._max_proc
            ...
            future_to_lib_name = {executor.submit(
                    ...
                    size_factor, wiggle_folder): lib.lib_name for lib,
                    size_factor in zip(self._lib_dict.values(),
                    self._size_factors)}
        for future in futures.as_completed(future_to_lib_name):
            lib_name = future_to_lib_name[future]
    ...
    def _generate_normalized_wiggle_file_for_lib(self, lib, size_factor,
        ...
        wiggle_writers = dict([(strand, WiggleWriter("%s_%s" % (lib.lib_name,
                        strand), open("%s/%s_div_by_%s_%s.wig" % (wiggle_folder,
                        lib.lib_name,
                        size_factor,
                        strand), "w"))) for strand in strand_dict.values()])
        for replicon in self._replicon_dict:
            for strand in strand_dict.keys():
                if strand == "-":
                    ...
                    factor = 1.0/size_factor
                try:
                    wiggle_writers[strand_dict[strand]
                            ].write_replicons_coverages(replicon, 
                            lib.replicon_dict[replicon]["coverages"][strand],
                            factor = factor)
                except Exception as exc:
        ...
        with open("%s/peaks_%s.gff" % (self._output_folder, replicon),
            ...
            out_gff_fh.write("##gff-version 3\n"
                    ...
                    self._replicon_dict[replicon]
                            ['seq_start_pos'] + 1,
                    self._replicon_dict[replicon]['seq_end_pos'],
                    '\n'.join(df.apply(self._write_gff_entry, axis=1)),
    ...
    def _convert_to_data_frame(self):
        ...
        print("Removing empty windows from DataFrame with %s rows..." % len(self._window_df.index), flush=True)
        t_start = time()
        self._window_df = self._window_df.loc[(self._window_df.loc[:,
                self._lib_names_list].sum(axis = 1) > 0), :]
        t_end = time()
        print("Removal took %s seconds. DataFrame contains now %s rows." % ((t_end-t_start), len(self._window_df.index)), flush=True)
        if self._window_df.empty:
            print("**Dataframe empty**", flush=True)
            return
        # define size factors
        print("Calculating size factors and normalizing libraries...", flush=True)
        t_start = time()
        if self._norm_method == "tmm":
            # calc size factors based on tmm using windows with expression
            # in control
            tmm_df = self._window_df.loc[self._window_df.loc[:,
                    self._ctr_lib_list].max(axis = 1) > 0,
            ...
            count_df = self._window_df.loc[self._window_df.loc[
                    :, self._ctr_lib_list].max(axis = 1) > 0,
                    self._lib_names_list]
            # if data frame with reads in the control is empty skip
            # normalization
        ...
        self._window_df["fold_change"] = (self._window_df.loc[:, 
                self._exp_lib_list].sum(axis = 1) /
        ...
        self._window_df = self._filter_windows_df(
                self._window_df)
    ...
    def _filter_windows_df(self, df):
        ...
        print("Removing windows where not all experiment libs show expression from DataFrame with %s rows..." % len(df), flush=True)
        t_start = time()
        ...
        t_end = time()
        print("Removal took %s seconds. DataFrame contains now %s rows." % ((t_end-t_start), len(df)), flush=True)
        if df.empty:
            return df
        # minimum expression cutoff based on mean over experiment libraries
        print("Removing windows based on mad cutoff from DataFrame with %s rows..." % len(df), flush=True)
        t_start = time()
        median_abs_dev_from_zero = mad(df.loc[:, self._exp_lib_list].mean(
                axis = 1), center = 0.0)
        min_expr = (self._mad_multiplier * median_abs_dev_from_zero)
        print("Minimal window expression based on mean over RIP/CLIP libraries:"
              "%s (MAD from zero: %s)" % (min_expr, median_abs_dev_from_zero),
              flush=True)
        df = df.loc[df.loc[:, self._exp_lib_list].mean(axis = 1)
                >= min_expr, :]
        t_end = time()
        print("Removal took %s seconds. DataFrame contains now %s rows." % ((t_end-t_start), len(df)), flush=True)
        if df.empty:
            return df
        print("Removing windows where experiment expression is lower than control expression from DataFrame with %s rows..." % len(df), flush=True)
        t_start = time()
        if self._pairwise_replicates:
            # experiment expression must be larger than respective control
            # for each library pair
            for exp_lib, ctr_lib in zip(self._exp_lib_list,
                    self._ctr_lib_list):
                df = df.loc[(df.loc[:, exp_lib] > df.loc[:, ctr_lib]), :]
        else:
            # minimum experiment expression larger than maximum control expression
            df = df.loc[df.loc[:, self._exp_lib_list].min(
                axis = 1) > df.loc[:, self._ctr_lib_list].max(
                axis = 1), :]
        t_end = time()
        print("Removal took %s seconds. DataFrame contains now %s rows." % ((t_end-t_start), len(df)), flush=True)
        if df.empty:
            return df
        # minimum fold change
        print("Removing windows based on minimum fold change from DataFrame with %s rows..." % len(df), flush=True)
        t_start = time()
        #df = df.loc[(df.loc[:, "fold_change"] >= self._fc_cutoff), :]
    ...
    def _check_significance_with_repl(self, p_and_padj_values):
        ...
        if (p_and_padj_values.loc["heterogenous_G_p_value"]
            ...
            return True
        if ((p_and_padj_values.loc["total_G_padj_value"] < self._padj_threshold)
                and ((replicate_G_p_values < self._rep_pair_p_val_threshold
                ).all())):
            return True
        return False
    ...
    def _combine_windows(self, df):
        ...
        for index, window in df.iterrows():
            ...
            # non-significant window
                ...
                # +1 -> combine adjacent peaks
                if (peak["peak_start"] is None or
                    window.loc["w_start"] <= peak["peak_end"] + 1):
                    continue
    ...
    def _get_overlap(self, peak_start, peak_end, feature_start, feature_end):
        return max(0,
                min(peak_end, feature_end) - max(peak_start, feature_start) + 1)