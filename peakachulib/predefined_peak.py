...
class PredefinedPeakApproach(object):
    ...
    def generate_combined_bed_file(self):
        ...
        with futures.ProcessPoolExecutor(max_workers=self._max_proc
            ...
            future_to_lib_name = {executor.submit(lib.merge_reads):
        ...
        for replicon in sorted(self._replicon_dict):
            ...
            split_index = pd.DataFrame(list(self._replicon_dict[replicon][
                    "reads"]["index"].str.split(',')), columns = ["start",
                    "end", "strand"])
            ...
            self._replicon_dict[replicon]["reads"] = split_index.join(
                    self._replicon_dict[replicon]["reads"]).sort(["strand",
                    "start", "end"], ascending=[False, True, True])
            self._replicon_dict[replicon]["reads"]["replicon"] = replicon
            output_df = output_df.append(self._replicon_dict[replicon]["reads"], 
                    ignore_index=True)
    ...
    def _split_cluster_peaks(self, block_df, cluster_expr, peak_df):
            ...
            if max_block_expr/cluster_expr < 0.01:
                return peak_df
            min_overlap = round((block_df.loc[max_block_ix, "blockEnd"] -
                    block_df.loc[max_block_ix, "blockStart"]) * 0.5)
            min_perc_of_max_block = 0.1
            
            overlaps = (block_df.loc[:, "blockEnd"].apply(
                    min, args=(block_df.loc[max_block_ix, "blockEnd"],)) - 
                    block_df.loc[:, "blockStart"].apply(
                    max, args=(block_df.loc[max_block_ix, "blockStart"],))
                    ).apply(max, args=(0,))
            ...
            peak_blocks = peak_blocks.loc[(peak_blocks["blockExpression"] / 
            ...
            peak_df = peak_df.append(pd.Series([peak_start, peak_end], index=[
                    "peak_start", "peak_end"]), ignore_index=True)
            return self._split_cluster_peaks(next_block_df, cluster_expr,
                    peak_df)
    ...
    def run_deseq2_analysis(self):
        count_df = self._peak_df.loc[:, self._exp_lib_list +
                self._ctr_lib_list]
        run_deseq2 = RunDESeq2(count_df, self._exp_lib_list, self._ctr_lib_list)
        ...
        self._plot_initial_peaks(self._peak_df.baseMean, np.power(2.0,
                self._peak_df.log2FoldChange))
        # filter windows
    ...
    def _filter_peaks(self, df):
        ...
        print("Removing windows based on padj from DataFrame with %s rows..." % len(df), flush=True)
        t_start = time()
